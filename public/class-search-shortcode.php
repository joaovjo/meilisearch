...
class Meilisearch_Search_Shortcode
{
	 ...
	 */
	private function render_results_counter(
		string $search_query,
		int $total_results,
		int $paged,
	...
	private function render_result_item(array $hit, array $atts): void
	{
		...
		?>
		<article class="meilisearch-result-item" data-blog-id="<?php echo esc_attr($blog_id); ?>" data-post-type="<?php echo
			esc_attr($post_type)
		; ?>">
			<header class="meilisearch-result-header">
				<h2 class="meilisearch-result-title">
					...
					<?php if ($ranking_score > 0): ?>
						<span class="meilisearch-relevance-badge meilisearch-relevance-<?php echo esc_attr($relevance_class); ?>" title="Relevância: <?php echo
				...
				<?php if (
					'yes' === $atts['show_date'] && !empty($post_date)
					|| 'yes' === $atts['show_author'] && !empty($author_name)
				): ?>
	...
	private function render_pagination(int $total_results, int $paged, int $results_per_page, string $search_query): void
	{
		...
		?>
		<nav class="meilisearch-pagination" role="navigation" aria-label="<?php esc_attr_e(
